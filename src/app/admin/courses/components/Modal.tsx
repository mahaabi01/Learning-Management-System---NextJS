...
const Modal: React.FC<IModalProps> = ({ closeModal, courseId }) => {
  const dispatch = useAppDispatch(); //disparch redux actions
  const [loading, setLoading] = useState(false); //tracks whether the form submission is in progress
  const { status } = useAppSelector((store) => store.lessons); //gets the lesson creation status
  const { categories } = useAppSelector((store) => store.categories); //retrieve course categories from Redux state

  //define the initial lesson data
  const [data, setData] = useState<ILessonForData>({
    title: "",
    description: "",
    videoUrl: "",
    course: courseId, //courseId is pre-filled to link the lesson with the correct course
  });
